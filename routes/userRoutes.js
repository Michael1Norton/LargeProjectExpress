...
router.post("/forgot-password", async (req, res) => {
  ...
  const mailOptions = {
    ...
    text: `You are receiving this because you have requested the reset of the password for your Culinary Canvas account.\n\nPlease click on the following link, or paste this into your browser to complete the process:\n\nhttp://${req.headers.host}/reset-password\n\n`,
  };
  ...
  transporter.sendMail(mailOptions, (err) => {
    if (err)
      return res
        .status(500)
        .send({ message: "Error sending email", email: user.email });
